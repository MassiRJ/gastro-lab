...
export default function Menu({ items, onAddToCart }) {
  // Protección: Si items no existe, usamos un array vacío para que no rompa la página
        ...
        {safeItems.map((item) => (
          <div key={item.id} className="bg-zinc-900/50 border border-white/5 rounded-2xl overflow-hidden hover:border-emerald-500/30 transition-all group">
            <div className="h-48 bg-zinc-800 relative overflow-hidden flex items-center justify-center">
               <span className="text-4xl font-bold text-zinc-700 group-hover:scale-110 transition-transform duration-500">
                 {item.title.charAt(0)}
               </span>
            </div>
            <div className="p-6">
              <div className="flex justify-between items-start mb-2">
                ...
                <span className="text-emerald-400 font-mono font-bold">S/ {item.price.toFixed(2)}</span>
              </div>
