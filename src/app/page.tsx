...
// --- YA NO USAMOS CONSTANTES FIJAS, AHORA VIENEN DE LA BD ---
...
export default function Home() {
  ...
  // --- NUEVO: ESTADO PARA EL MENÚ DINÁMICO ---
  const [menuItems, setMenuItems] = useState([]);
  const [loadingMenu, setLoadingMenu] = useState(true);

  // --- CARGAR MENÚ DE SUPABASE ---
  useEffect(() => {
    const fetchMenu = async () => {
      ...
      if (data && data.length > 0) {
        ...
        // Fallback si la DB está vacía: usar las constantes para que no se vea feo
            ...
            {loadingMenu ? (
                ...
                <div key={item.id} className="group cursor-pointer">
                    ...
                    <div className="relative px-2">
                        ...
                        <p className="text-zinc-500 text-sm mb-6 leading-relaxed min-h-[40px]">{item.desc || "Plato especial de la casa."}</p>
                        <button onClick={() => addToCart(item)} className="w-full border border-zinc-800 hover:bg-white hover:text-black hover:border-white text-white py-4 font-bold flex items-center justify-center gap-3 transition-all text-xs uppercase tracking-[0.2em]"><Plus size={16} /> Agregar</button>
                    </div>
                </div>
                ))
            )}
