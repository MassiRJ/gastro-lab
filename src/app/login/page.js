...
// Componente interno para manejar los params (Evita errores de hidratación)
function LoginForm() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const returnUrl = searchParams.get("returnUrl") || "/caja"; // <--- AQUÍ ESTÁ LA MAGIA
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Redirigir a la URL que estaba intentando visitar (o Caja por defecto)
    ...
    <div className="bg-zinc-950/80 backdrop-blur-md border border-zinc-800 p-8 rounded-2xl shadow-2xl">
        <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
                ...
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="w-full bg-black border border-zinc-800 text-white pl-12 pr-4 py-3 rounded-xl focus:border-amber-500 outline-none transition-all"
                        required
                    />
                </div>
            </div>
            ...
            <button 
                type="submit" 
                disabled={loading}
                className="w-full bg-amber-500 hover:bg-amber-400 text-black font-bold py-4 rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2 uppercase tracking-widest text-sm disabled:opacity-50"
            >
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-6 relative overflow-hidden">
      <div className="absolute inset-0 z-0">
        ...
        <img src="https://images.unsplash.com/photo-1559339352-11d035aa65de?q=80&w=1920&auto=format&fit=crop" className="w-full h-full object-cover opacity-20" alt="Background"/>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        {/* Suspense es necesario para usar useSearchParams en Next.js */}
        <Suspense fallback={<div className="text-white text-center">Cargando...</div>}>
            <LoginForm />
        </Suspense>

        <p className="text-center text-zinc-600 text-xs mt-8">&copy; 2025 GastroLab System. Acceso restringido.</p>
      </div>
    </div>
  );
}